...
- The optional `/resume` page (see `config/resume.config.yml`) is indexable only when it is enabled **and** global indexing is on; otherwise it serves with `noindex, nofollow` and is omitted from the sitemap. When disabled it renders the not-found page instead.
