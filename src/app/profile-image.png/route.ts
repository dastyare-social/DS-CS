...
// streamed fresh on every request; the default baked into the image at
// /app/public/profile-image.png is the fallback. No restart is needed after
...
async function readAvatar(): Promise<Buffer> {
  try {
    ...
    return readFile(path.join(process.cwd(), "public", "profile-image.png"));
  }
}
